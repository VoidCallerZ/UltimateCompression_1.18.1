...
public class UcAxeItem extends AxeItem
{
    public UcAxeItem(Tier pTier, int pAttackDamageModifier, float pAttackSpeedModifier, Properties pProperties)
    {
    ...
    public void inventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected)
    {
        if (pEntity instanceof Player)
        {
            ...
            if (pIsSelected)
            {
                if (pStack.getItem() == ToolRegistration.COMPRESSED_WOODEN_AXE.get() || pStack.getItem() == ToolRegistration.COMPRESSED_STONE_AXE.get())
                {
                    player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 1, 0));
                }
                else if (pStack.getItem() == ToolRegistration.COMPRESSED_IRON_AXE.get() || pStack.getItem() == ToolRegistration.COMPRESSED_DIAMOND_AXE.get())
                {
                    player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 1, 1));
                }
                else if (pStack.getItem() == ToolRegistration.COMPRESSED_GOLD_AXE.get())
                {
                    player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 1, 2));
                }
                else if (pStack.getItem() == ToolRegistration.COMPRESSED_NETHERITE_AXE.get())
                {
                    player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 1, 1));
                }
            }
    ...
    {
        if (itemStack.getItem() == ToolRegistration.COMPRESSED_WOODEN_AXE.get())
        {
            return 1800;
        }
        return 0;
