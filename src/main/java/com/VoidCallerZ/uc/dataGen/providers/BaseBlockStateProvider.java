...
public abstract class BaseBlockStateProvider extends BlockStateProvider {
    ...
    protected void singleTextureBlockWithBlockColors(Block block, String modelName, String textureName)
    {
        ModelFile model = models().singleTexture(modelName, mcLoc(BLOCK_FOLDER + "/leaves"), "all", modLoc(textureName));
        simpleBlock(block, model);
    }
    ...
    protected void paneBlock(Block block, String modelName, String textureName)
    {
        paneBlock((IronBarsBlock) block, modelName, modLoc(textureName), extend(modLoc(textureName), "_pane_top"));
    }
