...
public abstract class BaseLootTableProvider extends LootTableProvider
{
    ...
    protected LootTable.Builder createGlassBlockTable(String name, Block block)
    {
        LootPool.Builder builder = LootPool.lootPool()
                .name(name)
                .setRolls(ConstantValue.exactly(1))
                .add(LootItem.lootTableItem(block).when(MatchTool.toolMatches(ItemPredicate.Builder.item().hasEnchantment(new EnchantmentPredicate(Enchantments.SILK_TOUCH, MinMaxBounds.Ints.atLeast(1))))));
        return LootTable.lootTable().withPool(builder);
    }
